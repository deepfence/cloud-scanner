...
func GetAWSFargateMetadata(onlyValidate bool) (CloudMetadata, error) {
	...
	awsFargateMetadata.Region = RemoveLastCharacter(awsFargateMetadata.Zone)
...
func GetAWSMetadata(onlyValidate bool) (CloudMetadata, error) {
	client := &http.Client{Timeout: 10 * time.Second}
	awsMetadata := CloudMetadata{CloudProvider: CloudProviderAWS, Label: "AWS"}
	// Get token
...
func GetGoogleCloudMetadata(onlyValidate bool) (CloudMetadata, error) {
	client := &http.Client{Timeout: 10 * time.Second}
	gcpMetadata := CloudMetadata{CloudProvider: CloudProviderGCP}
	...
	gcpMetadata = CloudMetadata{CloudProvider: CloudProviderGCP, ID: accountID, Label: "Google Cloud", InstanceID: strconv.FormatInt(gcMetadataAll.ID, 10), Hostname: gcMetadataAll.Hostname, Name: gcMetadataAll.Name, Zone: gcMetadataAll.Zone, Region: region, MachineType: gcMetadataAll.MachineType}
	var privateIP []string
	var publicIP []string
...
func GetAzureMetadata(onlyValidate bool) (CloudMetadata, error) {
	client := &http.Client{Timeout: 10 * time.Second}
	azureMetadata := CloudMetadata{CloudProvider: CloudProviderAzure}
	...
	azureMetadata = CloudMetadata{CloudProvider: CloudProviderAzure, ID: azureMetadataAll.Compute.SubscriptionID, Label: "Azure", VmID: azureMetadataAll.Compute.VMID, Name: azureMetadataAll.Compute.Name, VMSize: azureMetadataAll.Compute.VMSize, Region: azureMetadataAll.Compute.Location, Zone: azureMetadataAll.Compute.Zone, OsType: azureMetadataAll.Compute.OsType, SKU: azureMetadataAll.Compute.Sku, ResourceGroupName: azureMetadataAll.Compute.ResourceGroupName}
	var privateIP []string
	var publicIP []string
...
func GetGenericMetadata(onlyValidate bool) (CloudMetadata, error) {
	...
	ipAddress, _, err := GetDefaultRouteIPAddress()
	if err != nil {
